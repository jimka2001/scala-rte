...
object Adjuvant {
  ...
  def biasedGaussian(n: Int, m: Int, biasFactor: Double = 6.0): Int = {
    ...
    // https://chatgpt.com/share/68d79856-7fb8-800d-a790-d6db87723549
    require(n < m, "Lower bound n must be less than upper bound m")
    require(biasFactor > 0)
    ...
    if (range <= 1)
      ...
      val sigma = range.toDouble / biasFactor
      var x = range // initialize outside valid range
      // sample until within half-range
      while (x >= range / 2) {
        x = math.abs(random.nextGaussian() * sigma).toInt
      }
