\section{Embedding a Type System}

{  \setbeamercolor{background canvas}{bg=chaptercolor}
\begin{frame}{Challenge \textnumero 3}{Type Lattice}
...
\begin{frame}{Example \code{SimpleTypeD} Expression Tree}
  \usebox\tdast
  ...
  \item Each node is an instance of class \code{SimpleTypeD}.
  \item Leaf nodes interface to Scala classes via \code{SAtomic(...)}.
  \item .. and to literal Scala values via \code{SEql(...)}
  \end{itemize}
\end{frame}


\begin{frame}{\code{SimpleTypeD} class quasi-ADT}
  \scalebox{0.9}{\input{type-adt}}
